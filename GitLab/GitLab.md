...
detached - the pipeline ran only on the source branch of the merge
commit\
merged results - runs on the result of combining the source branch
changes with target branch (I read somewhere that the change has still
not been merged into target as yet)\
merge train - runs when merging multiple merge requests at the same
time\
\
\
\
...
- merge_requests - only run the job when a merge request has been
created\
except\
- schedules - don't run the job if pipeline is kicked off from a
schedule\
...
url: \<url\>\
on_stop: \<job-name\>\
when: - manual *the job needs to be kicked off manually. \[ALSO IT SEEMS
THAT GITLAB CAN RUN THE JOB AFTER MERGE REQUEST WHEN SOURCE BRANCH IS
BEING DELETED\]* - never\
\
allow_failure: \[true \| false\] - allow_failure will determine whether
a pipeline should continue running when a job fails\
...
GIT_STRATEGY: none - when this variable is defined in a job, GitLab will
not clone the repo in the job run\
\
\
\
